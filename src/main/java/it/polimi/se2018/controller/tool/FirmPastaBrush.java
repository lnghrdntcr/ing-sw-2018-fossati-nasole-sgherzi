...
import it.polimi.se2018.controller.states.PlaceRedrawedDiceState;
    ...
    public State use(Controller controller, GameTableMultiplayer model, TurnState state, Event event) {
        try {
            ...
            if (model.getPlayerSchemaCopy(ev.getPlayerName()).isDiceAllowedSomewhere(redrawed, SchemaCardFace.Ignore.NOTHING)) {
                ...
                return new PlaceRedrawedDiceState(controller, new TurnState(controller, state.isDicePlaced(), true), redrawed, ev.getPlayerName(), model.getDiceNumberOnDraftBoard()-1);
            } else {
